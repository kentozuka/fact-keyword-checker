...
export class YahooShoppingPage extends BasicPage {
  readonly page: Page
  ...
  constructor(page: Page) {
    super()
    this.page = page
    this.link = 'https://shopping.yahoo.co.jp/'
    this.searchBox = page.getByPlaceholder('何をお探しですか？')
    this.suggestContainer = page.locator('[class^=SearchBox__suggestList___]')
    this.suggestions = page.locator('[class^=SearchBox__suggestItem____]')
  }
  ...
  async screenshot(owner: string, keyword: string) {
    ...
    await this.suggestContainer.screenshot({
      path: `images/${ymd}_${owner}_${keyword}_yahooShopping.png`
    })
  }
